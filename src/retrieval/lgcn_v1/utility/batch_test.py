...
def inference(sess, model, users_to_test):
    u_batch_size = BATCH_SIZE
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        rate_batch = sess.run(model.batch_ratings, {model.users: user_batch,
                                                    ...
                                                    model.mess_dropout: [0.] * len(eval(args.layer_size))})
        rate_batch = np.array(rate_batch)  # (B, N)
        all_result.append(rate_batch)

    all_result = np.array(all_result)
    print(all_result.shape)  # (1, testset_uidsize, testset_itemsize)

    with open('./lgcn_tf_test_result.txt', 'w') as wf:
        for uid_idx, pred_res in zip(users_to_test, all_result[0]):
