...
def load_target_valid_test(i=1):
    df_test_run = pd.read_csv('/home/workspace/DATA/t{}/test_run.tsv'.format(i), sep='\t', header=None,
                              names=['userId', 'itemId'])
    df_valid_run = pd.read_csv('/home/workspace/DATA/t{}/valid_run.tsv'.format(i), sep='\t', header=None,
                               names=['userId', 'itemId'])
    ...
    df_test_run = df_test_run.set_index('userId')['itemId'].apply(pd.Series).stack().reset_index().rename(
        columns={0: 'itemId'}).loc[:, ['userId', 'itemId']]
    df_valid_run = df_valid_run.set_index('userId')['itemId'].apply(pd.Series).stack().reset_index().rename(
        columns={0: 'itemId'}).loc[:, ['userId', 'itemId']]
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    t0 = time()
    if args.pretrain == -1 or args.inference == 1:
        print('->[inference]using pretrain user&item embeddings')
    ...
    *********************************************************
    ...
    if args.save_flag == 1:
        ...
        save_saver = tf.train.Saver(max_to_keep=20)
    ...
    *********************************************************
    ...
    if args.pretrain == 1 or args.inference == 1:
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            print('->[restore]load the pretrained model parameters from: ', pretrain_path)

            # inference
            if args.inference == 1:
                print('->[inference] predict full testset scores...')
                users_to_test = list(data_generator.test_set.keys())
                ret = inference(sess, model, users_to_test)
                sys.exit()
    ...
    *********************************************************
    ...
    _item_vocab = pd.read_csv('./Data/s123t12_withoutqrel/item_list.txt', sep=' ', header=0)
    _user_vocab = pd.read_csv('./Data/s123t12_withoutqrel/user_list.txt', sep=' ', header=0)
    _item_vocab = dict(zip(_item_vocab['org_id'], _item_vocab['remap_id']))
    _user_vocab = dict(zip(_user_vocab['org_id'], _user_vocab['remap_id']))

    users_to_test = [_user_vocab[uu] for uu in users_to_test]
    inference(sess, model, users_to_test)

    # test/valid set
    df_t1_valid, df_t1_test = load_target_valid_test(1)
    df_t2_valid, df_t2_test = load_target_valid_test(2)

    # restore predict score
    itemlist = list(_item_vocab.keys())
    userlist = list(_user_vocab.keys())
    print('itemlist:{} userlist:{}'.format(len(itemlist), len(userlist)))

    pred_res = pd.read_csv('./lgcn_tf_test_result.txt', sep='\t', header=None, names=['userId', 'itemScoreList'])
    ...
    if not os.path.exists(dirpattern):
        print('->Create directory: {}'.format(dirpattern))
        os.mkdir('{}'.format(dirpattern))
        os.mkdir('{}/t1'.format(dirpattern))
        os.mkdir('{}/t2'.format(dirpattern))
    formup_submittion(df_t1_valid, pred_res_dic, '{}/t1/valid_pred.tsv'.format(dirpattern))
    formup_submittion(df_t1_test, pred_res_dic, '{}/t1/test_pred.tsv'.format(dirpattern))
    formup_submittion(df_t2_valid, pred_res_dic, '{}/t2/valid_pred.tsv'.format(dirpattern))
    formup_submittion(df_t2_test, pred_res_dic, '{}/t2/test_pred.tsv'.format(dirpattern))
    ...
    global_score = pd.concat([ttmp_1, ttmp_2], axis=1)
    global_score = global_score.loc[:, ['userId', 'itemId', 'itemScoreList']].rename(
    ...
    Cache.dump_pkl(global_score, f'{dirpattern}/t1/test_pred_all.pkl')
    Cache.dump_pkl(global_score, f'{dirpattern}/t2/test_pred_all.pkl')

    # # best epoch.
    # if not len(rec_loger):
    #     print('->[Exit]empty recall/precision/ndcg logger. none validation records.')
    #     sys.exit()
    #
